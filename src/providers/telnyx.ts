import { FaxProvider, ProviderContext, ProviderSendResult, SendFaxParams } from "./types";
...
export const TelnyxProvider: FaxProvider = {
    ...
    async sendFax(params: SendFaxParams, ctx: ProviderContext): Promise<ProviderSendResult> {
        const { env, baseUrl, log } = ctx;

        if (!env.TELNYX_API_KEY) {
            return { ok: false, status: 500, fax: null, raw: { error: "TELNYX_API_KEY not configured" } };
        }
        if (!env.CONNECTION_ID) {
            return { ok: false, status: 500, fax: null, raw: { error: "CONNECTION_ID not configured" } };
        }
        if (!env.FAX_FROM) {
            return { ok: false, status: 500, fax: null, raw: { error: "FAX_FROM not configured" } };
        }
        ...
        log(ctx.requestId, "telnyx fax response", { status: telnyxRes.status, body });
    ...
    async handleWebhook(req: Request, ctx: ProviderContext): Promise<Response> {
        ...
        try {
            ...
            if (eventType === "fax.received") {
                ...
                if (mediaUrl) {
                    log(ctx.requestId, "downloading fax media", { mediaUrl: mediaUrl.slice(0, 120) });
            ...
            return new Response(JSON.stringify({ ok: true, requestId: ctx.requestId }), {
                status: 200,
            ...
            return new Response(JSON.stringify({ ok: false, requestId: ctx.requestId, error: String(err) }), {
                status: 500,
                headers: { "content-type": "application/json" },
            });
        }
