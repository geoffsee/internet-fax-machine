...
describe("TelnyxProvider.sendFax", () => {
    ...
    it("posts fax payload to Telnyx with required fields", async () => {
        ...
        expect(captured.body.media_url).toBe("https://worker.example/media/file.pdf");
        expect(captured.body.webhook_url).toBe("https://worker.example/telnyx/fax-rx");
    });
...
describe("TelnyxProvider.handleWebhook", () => {
    ...
    it("stores received fax media into KV", async () => {
        ...
        globalThis.fetch = async (_url: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
            expect(init?.headers && (init.headers as any).authorization).toBe("Bearer key_123");
