...
export class FaxWorkerClient {
  ...
  async uploadMedia(
    ...
    const response = await fetch(
      `${this.workerUrl}/media/${encodeURIComponent(key)}`,
      {
        method: "PUT",
  ...
  async sendFax(to: string, mediaKey: string): Promise<SendFaxResponse> {
    ...
    if (!response.ok) {
      throw new Error(
        `Fax send failed (${response.status}): ${JSON.stringify(result)}`,
      );
    }
   ...
   */
  async sendFaxWithFile(
    to: string,
    file: Buffer | Uint8Array,
    filename = "document.pdf",
  ): Promise<SendFaxResponse> {
  ...
  async uploadAndSendFax(
    ...
    console.log(
      `Uploaded: ${uploadResult.mediaUrl} (${uploadResult.bytes} bytes)`,
    );
