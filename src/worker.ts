...
import { DropboxFaxProvider } from "./providers/dropbox";
import { SinchProvider } from "./providers/sinch";
...
export default {
  async fetch(
    ...
    if (url.pathname.startsWith("/media/") && req.method === "PUT") {
      ...
      log(requestId, "media uploaded", {
        key,
        bytes: bytes.byteLength,
        mediaUrl,
      });
      return new Response(
        JSON.stringify({ ok: true, key, mediaUrl, bytes: bytes.byteLength }),
        {
          status: 200,
    ...
    if (url.pathname === "/fax/send" && req.method === "POST") {
      ...
      if (contentType.includes("multipart/form-data")) {
        ...
        log(requestId, "uploaded outbound fax media", {
          key,
          bytes: bytes.byteLength,
        });
      } else {
        ...
        if (!to) return json({ ok: false, error: "missing 'to' field" }, 400);
        if (!body.media_key)
          return json({ ok: false, error: "missing 'media_key' field" }, 400);
      ...
      return json(
        { ok: sendResult.ok, requestId, fax: sendResult.fax },
        sendResult.status,
      );
    }
...
function log(requestId: string, message: string, meta?: unknown) {
  console.log(
    JSON.stringify({ ts: new Date().toISOString(), requestId, message, meta }),
  );
}
...
function checkBasicAuth(
  req: Request,
  env: WorkerEnv,
): { authorized: boolean; user?: string } {
...
function resolveProvider(
  env: WorkerEnv,
): { ok: true; provider: FaxProvider } | { ok: false; error: string } {
  ...
  switch (name) {
    ...
    case "dropbox":
    case "dropbox-fax":
      return { ok: true, provider: DropboxFaxProvider };
    case "sinch":
      return { ok: true, provider: SinchProvider };
    default:
